...
int pindown_inode_permission(struct inode *inode, int mask, struct nameidata *nd)
{
	...
	//printk("Get the process security info.\n");

	/* Get the inode policy */
	// FILL IN
  	inode_policy = get_inode_policy(inode, "security.pindown");
	//printk("Get the inode policy.\n");

        /* Compare process security info to inode policy */
	// FILL IN
  	if (sec == NULL){
		printk(KERN_INFO "sec is null : Access denied.\n");
    		rc = -EACCES;
    		goto out;
  	}
	else{
    		if (inode_policy == NULL || !strcmp(sec->bprm_pathname, inode_policy)){
      			printk(KERN_INFO "Access allowed.\n");
      			goto out;
  		}
		else{
  			printk(KERN_INFO "strcmp failed: Access denied.\n");
  			rc = -EACCES;
			goto out;
		}
	}
out:
	...
	//printk("Finished Comparing process security info to inode policy");
	return rc;
}
...
int pindown_task_alloc_security(struct task_struct * p)
{
	...
	// FILL IN
  	if (current->security != NULL) {
		...
		memcpy(sec->bprm_pathname, parent_sec->bprm_pathname, MAX_PATHLEN);
...
int pindown_bprm_set_security(struct linux_binprm * bprm)
{
	...
	if (!rc) {		// FILL IN
      		if (bprm == NULL){
	    		...
	    		sec->pathlen = strlen(bprm->filename);
    		}
	}
